...
export default async function OrderDetailPage({ params }: PageProps) {
  ...
  // WhatsApp customer button (plan-gated)
  const whatsappEnabled = await hasFeature(tenantId, "whatsapp");
  const showWhatsApp = whatsappEnabled && order.phone;
  const whatsappUrl = showWhatsApp
    ? (() => {
        const msg = t.whatsapp.orderContactCustomerMessage
          ...
          <div className="space-y-3">
            ...
            {whatsappUrl && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="mt-2 flex items-center justify-center gap-2 w-full rounded-xl bg-[#25D366] py-3 text-white font-semibold text-sm hover:bg-[#1ebe57] active:scale-[0.98] transition-transform"
              >
