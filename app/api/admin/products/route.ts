...
function parseSizes(value: unknown): any {
  ...
  throw new ApiError(
    400,
    "BAD_REQUEST",
    "sizes must be an object, array, or comma-separated string",
  );
}
...
function parseCreatePayload(body: any): CreateProductPayload {
  ...
  const badges =
    body.badges === undefined ? undefined : parseBadges(body.badges);
  ...
  const isDualFormat =
    sizes && typeof sizes === "object" && "dimensions" in sizes;
  if (!isDualFormat && ((sizeSystem && !sizes) || (!sizeSystem && sizes))) {
    throw new ApiError(
      400,
      "BAD_REQUEST",
      "sizeSystem and sizes must both be provided",
    );
  }
