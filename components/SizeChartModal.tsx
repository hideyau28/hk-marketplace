        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-zinc-100">
              <th className="px-3 py-2 text-left font-medium text-zinc-600">US</th>
              <th className="px-3 py-2 text-left font-medium text-zinc-600">UK</th>
              <th className="px-3 py-2 text-left font-medium text-zinc-600">EU</th>
              <th className="px-3 py-2 text-left font-medium text-zinc-600">CM</th>
            </tr>
          </thead>
          <tbody>
            {sizes.map((size, idx) => (
              ...
              >
                <td className="px-3 py-2 text-zinc-900">{size.us}</td>
                <td className="px-3 py-2 text-zinc-900">{size.uk}</td>
                <td className="px-3 py-2 text-zinc-900">{size.eu}</td>
                <td className="px-3 py-2 text-zinc-900">{size.cm}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
export default function SizeChartModal({
  ...
  return (
    <>
      ...
      {/* Modal */}
      <div className="fixed inset-x-4 top-1/2 -translate-y-1/2 z-50 mx-auto max-w-lg bg-white rounded-2xl max-h-[80vh] flex flex-col shadow-xl">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4">
          {isKids ? (
            <>
              <SizeTable
                title={locale === "zh-HK" ? "Toddler (TD) - US 2C-10C" : "Toddler (TD) - US 2C-10C"}
                sizes={TODDLER_SIZES}
              />
              <SizeTable
                title={locale === "zh-HK" ? "Preschool (PS) - US 10.5C-3Y" : "Preschool (PS) - US 10.5C-3Y"}
                sizes={PRESCHOOL_SIZES}
              />
              <SizeTable
                title={locale === "zh-HK" ? "Grade School (GS) - US 3.5Y-7Y" : "Grade School (GS) - US 3.5Y-7Y"}
          ...
          {/* Tip */}
          <div className="mt-4 p-3 bg-zinc-50 rounded-xl text-xs text-zinc-600">
            {locale === "zh-HK"
