    ...
    <>
      <div className="sticky top-[57px] z-40 border-b border-zinc-200 bg-white/95 backdrop-blur dark:border-zinc-800 dark:bg-zinc-950/95">
        <div className="mx-auto max-w-6xl px-4 py-2.5">
          {/* Pills container with overflow-x-auto and hidden scrollbar */}
          <div className="flex items-center gap-2 overflow-x-auto scrollbar-hide [-webkit-overflow-scrolling:touch] [-ms-overflow-style:none] [scrollbar-width:none]">
            {/* Filter pill - olive green */}
            <button
              onClick={() => setIsFilterOpen(true)}
              className="rounded-full px-3 py-1.5 text-sm font-medium transition-colors shrink-0 bg-olive-600 text-white hover:bg-olive-700"
            >
              {isZh ? "篩選" : "Filter"}
            </button>
            ...
            {quickPills.map((pill) => (
              <button
                ...
                className={`rounded-full px-3 py-1.5 text-sm font-medium transition-colors shrink-0 whitespace-nowrap ${
                  pill.isHot
