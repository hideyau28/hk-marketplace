...
cat > 'app/[locale]/admin/orders/page.tsx' <<'EOM'
...
# 更新 TopNav（你專案用 src/components/TopNav.tsx）
TARGET="src/components/TopNav.tsx"
if [ -f "$TARGET" ]; then
  # 若已經加過 Admin link，就唔再重覆插入
  if ! grep -q '/admin/products' "$TARGET"; then
    perl -0777 -i -pe 's@(href=\{`/\$\{locale\}/cart`\}[^<]*>\\s*\\{t\\.nav\\.cart\\}\\s*</Link>)@$1\n          <Link className="text-white/70 hover:text-white" href={`/${locale}/admin/products`}>Admin</Link>\n          <Link className="text-white/70 hover:text-white" href={`/${locale}/admin/orders`}>Orders(Admin)</Link>@s' "$TARGET"
  fi
fi
